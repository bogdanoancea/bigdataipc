...
\begin{center}
	...
	$n$ = number of products within the same assortment based on the classification method, \\
	$i_{p_{v_{j}}}$ = the $j-th$ elementary price index within the same assortment
\end{center}

The subsequent calculations follow roughly the steps from the classical CPI methodology\cite{cpi}, taking into consideration that in order 
...
re-calibration procedure to aggregate price indices at expenditure group level using the following formula:
...
The re-calibration coefficient was applied to each weight from the classical CPI expenditure groups according to the formula:
...
\bibitem{hhs}
INS Romania, Coordonate ale nivelului de trai in Romania (in Romanian), 2019, page 52.
