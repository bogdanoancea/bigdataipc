...
\begin{algorithm}
\caption{Algorithm for data download}\label{alg:dl}
	\begin{algorithmic}[1]
	
	\State Read $URL's$	
	\Procedure{scraper}{$URL's$}
	\State $ncores \gets \Call{detectCores}$ \Comment{detect the number of cores on the system}
	\State \Call{createClusters}{$ncores$}
	\For{each URL}
	\State \Call{createProcess}
	\State \Call{visitUrl}{}
	\If{\Call{isDeadUrl}{URL}}
	\State  $deadUrl \gets URL$
	\State \Call{writeResults}{$data$}
	\EndFor
	\EndProcedure
	\end{algorithmic}
\end{algorithm}
