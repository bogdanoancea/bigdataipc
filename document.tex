...
consumer price index or to improve the classical CPI computation \cite{otawa2017}. We started our work by identifying and 
...
collection procedures, the underlaying technological support and skills are critical and a continuity plan should be 
devised if some data sources become unavailable, 
...
of applying and/or adapting the traditional consumer price index methodology \cite{cpi} to the new data sources.  A compromise to 
...
cost associated with developing and maintaining a large portion of the production pipeline it introduces a set of risks that cannot be
easily overcome. We explored several existing software solutions: 
Robot framework \cite{robot2018}, Scrapy \cite{scrapy1}, \cite{scrapy2} Apache Nutch \cite{nutch}, RSelenium \cite{rs1}, and Rvest \cite{rvest}. 
...
hypermarket segment in Romania, does not have a section dedicated to online transactions. After a detailed analysis of the sales turnover, geographic coverage and online channels availability we selected 4 sites for food, 
...
traditional CPI methodology as much as possible intact. A graphical representation of the data collection and processing is shown in figure \ref{fig:6}.
...
for this procedure and the results are presented in table \ref{table:1}. The best results, as it can be observed in \ref{table:1}, 
...
The process is being graphically described by figure \ref{fig:2}. After data pre-processing, i.e. testing 
...
%TODO graphic number of products per month per group(food, clothes, footwear)
...
%TODO Benchmark between different webscraping technologies
...
%TODO propose some modelling techniques (ex. weighted regression)
